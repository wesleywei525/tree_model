...
$Entrpy(Decision|Outlook = Overcast) =-(0/5) * log_2(0/5) - (5/5) * log_2(5/5)= 0$  
...
<p align="center">
<img src="img/tree-outlook.png", width=400px> 
</p>
  
__Split on overcast outlook__  
In the overcast node, all decisions are Yes. So the entropy of this node is 0 which make this node perfect purity and there will be no further spliting on this node.
...
Results show that we should use Humidity as the spliting variable for sunny outlook node. After spliting on Humidity, both children node reach perfect purity, so the spliting stop.
